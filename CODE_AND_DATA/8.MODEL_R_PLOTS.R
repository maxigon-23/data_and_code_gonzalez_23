...
data_obs_14_incub <- data_obs_14_incub %>%
  dplyr::filter(treatment=="R" & time =="2021") %>% 
...
pars <- c(median(MCMC_R_results$pars[,1]),
             median(MCMC_R_results$pars[,2]),
             median(MCMC_R_results$pars[,3]))


salida <- autoparam(pars)
...
sens_C_POM <- read_excel("sens_var_outputs/sesns_var_output_R.xlsx", sheet = "sens_C_POM") %>% 
  dplyr::rename(time = x) %>% 
  mutate(fraction = "C_POM",
         Mean = salida$C_POM)

sens_C_MAOM <- read_excel("sens_var_outputs/sesns_var_output_R.xlsx", sheet = "sens_C_MAOM") %>% 
  dplyr::rename(time = x) %>% 
  mutate(fraction = "C_MAOM",
         Mean = salida$C_MAOM)

sens_C_ha_mean <- read_excel("sens_var_outputs/sesns_var_output_R.xlsx", sheet = "sens_C_ha_mean") %>% 
  dplyr::rename(time = x) %>% 
  mutate(fraction = "C_bulk",
         Mean = salida$C_ha_mean)

sens_C14_bulk <- read_excel("sens_var_outputs/sesns_var_output_R.xlsx", sheet = "sens_C14_bulk") %>% 
  dplyr::rename(time = x) %>% 
  mutate(fraction = "C14_bulk",
         Mean = salida$C_14_bulk)

sens_C14_POM <- read_excel("sens_var_outputs/sesns_var_output_R.xlsx", sheet = "sens_C14_POM") %>% 
  dplyr::rename(time = x) %>% 
  mutate(fraction = "C14_POM",
         Mean = salida$C_14_POM)

sens_C14_MAOM <- read_excel("sens_var_outputs/sesns_var_output_R.xlsx", sheet = "sens_C14_MAOM") %>% 
  dplyr::rename(time = x) %>% 
  mutate(fraction = "C14_MAOM",
         Mean = salida$C_14_MAOM)

sens_C_14_CO2 <- read_excel("sens_var_outputs/sesns_var_output_R.xlsx", sheet = "sens_C_14_CO2") %>% 
  dplyr::rename(time = x) %>% 
...
plot_stocks <- ggplot() +
  ...
  xlab("Time (years)")+  guides(color=guide_legend(title="Fraction"),
                                fill=guide_legend(title="Fraction"))  +   theme(legend.position = "bottom")                 
...
plot_C14 <- ggplot() +
  ...
  geom_line(data=C_14_fractions_plot, aes(x=time, y=q50, color=fraction, size=fraction, alpha=fraction)) +
  ...
  scale_size_manual(values = c(Bulk=0.4, MAOM=0.4, POM=1, Efflux=0.4, Atmosphere=0.4))+
  scale_alpha_manual(values = c(Bulk=1, MAOM=1, POM=0.6, Efflux=1, Atmosphere=1))+
...
ggsave(plot=plot_grid, "plots/grid_R_model_output.png",device = "png" ,units="in", width=4, height=6.64, dpi=650)
